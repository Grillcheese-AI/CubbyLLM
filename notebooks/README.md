...
| [`grow_pilot.ipynb`](grow_pilot.ipynb) | **the growth pilot** (`docs/research/2026-09-25-grow-450m.md` §5). Four arms, one A100-hour each, all starting from base450m: the 450M continued (the control), width ×2, depth ×2 + FFN ×2 with zeroed exits (both grown exactly, as step 0 verified), and the same shape as a plain copy. It reads the arms at equal wall-clock and equal compute, projects each arm to 20 H100-hours, and writes the pre-registered verdict to Drive. Generated by [`_build_grow_pilot.py`](_build_grow_pilot.py); logic in `validation/grow_base.py` and `validation/train_base.py` (`CB_INIT_FROM`) | the base450m final checkpoint and the token-cache subset on Drive; a Colab A100-80G with high RAM, ~4.5 h; ~1 MB on Drive (the arms' checkpoints stay on local disk) |
