...
**v10t on LFM2.5-2.6B TRAINED (2026-09-04, Colab): Q4 at `standin/models/emitter_v10t.Q4_K_M.gguf` (sha256 `7541d6b5…`, script-ban cache built), Q8 on Drive. Val loss by family (nats/token, assistant span) vs the v9t LFM arm: toolcall **1.453** (new — the argument content: entity names and queries; the format is what the probe measures), chat 1.186 (=), history 1.160 (−0.15), quebec 1.146 (−0.30), appraisal 1.120 (+0.02), rewrite 0.235 (−0.06), emotion 0.230 (+0.08), affect 0.177 (−0.05), empathy 0.143 (+0.14 from a near-zero v9t), repair 0.107, identity 0.091 (+0.03), dialog_emotion 0.075, dialog_act 0.063, verbalize 0.036, safety 0.016, quebec_mc 0.006, content 0.001 — the two val samples are drawn per version, so ±0.05 is noise; no family moved the wrong way by more. **In-session tool probe (merged model, Colab): 3/3 calls when needed + 3/3 silent when not — MEETS THE BAR** in LFM's native format (`<|tool_call_start|>[news_search(query="quebec", language="en")]<|tool_call_end|>`; FR query kept in FR; a greeting, a fact question and a 'raconte-moi ta journée' stayed silent). Exact match by family (n=671; Δ vs the v9t LFM arm): content 1.000 (+0.00), chat 1.000 (+0.00), verbalize 1.000 (+0.00), quebec_mc 0.975 (+0.00), affect 0.975 (-0.03), toolcall 0.960, identity 0.938 (+0.00), safety 0.925 (+0.10), rewrite 0.850 (-0.05), dialog_act 0.850 (+0.03), dialog_emotion 0.750 (+0.03), empathy 0.725 (+0.27), repair 0.725 (+0.00), emotion 0.700 (+0.05), history 0.675 (+0.03), appraisal 0.475 (+0.42), quebec 0.450 (+0.33); by language en 0.819 / fr 0.809. **The Q4 probe first read 0/3 calls — a host bug, not the model:** llama-cpp's `create_completion` detokenizes with `special=False`, so LFM's control tokens `<|tool_call_start|>` / `<|tool_call_end|>` never reached the host and two of three calls arrived as a bare body (`[news_search(query="les nouvelles", language="fr")]`, parsed as a fragment). `LlamaCppEmitter._decode` now decodes token by token through `llm.generate()` and renders special tokens (same sampler defaults, the script ban, EOS + stop tokens by id); the probe's system prompt is the training-time `TOOL_SYSTEM` (every tool listed), as the notebook's probe. **Q4 read, CPU-only (`--n-gpu-layers 0`, 2–3 s a prompt, `standin/data/out/tool_call_probev10_lfm_cpu.json`): 3/3 calls when needed, 2/3 silent when not** — 'raconte-moi ta journée' produced `request_tool(name="remember_fact", …)`, a factory call for a tool that already exists, where the merged model stayed silent: one Q4-marginal decision, below the 3/3 + 3/3 bar by one. Host side a `request_tool` naming a registered tool is a no-op by design; data side v11 adds creative asks (stories, 'ta journée', songs) as request_tool NEGATIVES. **Confirmed three ways (`validation/logs/standin_v10_tool_call_probe_lfm{,_cpu,_q8_cpu}.json`): Vulkan Q4 3/3 + 2/3 (`request_tool(name="chat_status")`), CPU Q4 3/3 + 2/3 (`request_tool(name="remember_fact")`), CPU **Q8** 3/3 + 2/3 (`remember_fact(fact="ma chienne est nommée Luna")` — an INVENTED fact). Not quantization: the adapter, offered the full tool list, calls SOMETHING on a personal creative ask; which tool is noise. Two host rules already contain it at serve and are now named: (1) tools are offered only on turns the thalamus routes as needing them — 'raconte-moi ta journée' is `_NO_FACTS` → talk, no tool list in the prompt; (2) when the tool loop lands, a `remember_fact` argument must be grounded in the user's own turn (substring/entity check) or it is dropped, and a `request_tool` naming a registered tool is a no-op. Decision: **v10t serves as the talk adapter now** (it beat v9t on every weak family; the miss cannot reach a user on the talk path); the **kg_query tool loop waits for v11's request_tool/remember_fact negatives**, then the probe bar is re-read. **The Qwen arm is NOT run (owner's question 2026-09-09, answered with a read): the v9t Qwen GGUF, which never saw the toolcall family, given the same full tool list (`validation/logs/standin_v9_tool_call_probe_qwen_toolsys_cpu.json`, CPU): 1/3 calls (the other two collapse into `<tool_call>` stutters) and **0/3 silent** — `web_search` on 'what is the capital of australia', tag stutters on the greeting and on 'raconte-moi ta journée'. Its v9 single-tool read was already 2/3 + 0/3. So a tool list in the prompt over-triggers ANY talk-tuned base without negatives, the base does not decide it, and the 'free tool calling' reason to keep Qwen in the race is gone (LFM v10t: 3/3 + 2/3 at 127 tok/s vs 86). The Qwen arm stays in the notebook as a fallback, run only if LFM v11 still fails the silent half. Next: serve `--talk-gguf emitter_v10t.Q4_K_M.gguf --wiki` (Q8 also local: `emitter_v10t.Q8_0.gguf`); v11 negatives.**

**v11t BUILT (2026-09-09, `build_chat_sft.py --version v11`): 93,136 talk records (train rows after repeat 96,606, sha `fc1e1d78d799`), = v10t + the toolcall SILENT negatives: `toolcall` 3,234 rows, of which 2,563 `none` — 1,404 new silent negatives (1,143 creative chat rows re-issued with the tool list + 261 hand-written turns: story 41, day 40, feeling 39, joke 35, song 24, poem 22, opinion 20, small talk 20, personal statement 20; both formats, every reply through `voice_ok`). The check rejects a `request_tool` that names a registered tool. Notebook `notebooks/standin_v11_toolcall.ipynb` (LFM arm alone; four extra silent probe turns beside the unchanged six-turn bar). Local file `standin/data/out/emitter_sft_v11t.jsonl`; the Drive copy (`D:\My Drive\cubbyllm\standin\`) is the owner's step — Google Drive was not mounted when the set was built.**
...
- **The wiki world (2026-09-04, H-G6):** add `--wiki` to either entry point to mount `standin/data/wikikg.py`'s world — the wikikg-trajectories graph as 552,297 template facts (356,902 forward + 195,395 inverse readings, 163 relation types mapped = 96.6% of triples) in a `FactStore` with no cosine rows: lookup-first through its `TripleIndex`, an inverted-index IDF fallback at ~6 ms/query; 46 s and ~0.6 GB to build at startup (the Hub parquet is fetched to `standin/data/out/wikikg/` on first use). A question whose hop 0 is in ANY mounted world's index routes there at score 1.0 (`CubbyBrain.lookup_world`, why `lookup`). Measured (`validation/exp_g4b_wiki_world.py`): 12.5% of the graph's random-walk paths are functional two-hop questions (24,965 from 200k), **300/300 VM-verified to the one gold answer**; SimpleQA reachability is the sobering number — the gold answer is a graph entity for 19.9% of its 4,326 questions and lookup-reachable for 0.1%: the graph covers 45k articles, so SimpleQA moves only with search-and-learn.
