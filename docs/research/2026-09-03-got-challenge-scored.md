...
| (m) **retrieval as SEARCH vs retrieval as LOOKUP** (`validation/exp_m4_triple_lookup.py`, logs `exp_m4_triple_lookup.{json,log}`, 2026-09-04): the walk's acceptance test is EXACT on the parsed triple (tail string at hop 0, relation + subject after), but its candidates come from cosine top-3 above tau_ret; 95.7% of the store's facts parse as triples, so the same test runs as a (relation, subject) index lookup — no threshold, no k | per hop (1,126 hops, gold-chain entities): **cosine@3≥tau 78.7%**, cosine@3 83.4%, cosine@50 83.8%, **lookup 83.8%** (= the search ceiling), ambiguous (>1 candidate) **0.8%**; the other **16.2% of hops the plan's own acceptance test rejects the GOLD fact** — chain segmentation (the long relation phrases) and non-template/typo facts in the pool. End to end with the index alone: **581/800 correct (72.6%) vs the harvest's 517 verified (64.6%)** — the lookup recovers 64 of the 246 exhausted walks; the remaining 182 are parser-side, not retrieval | **the retrieval half of exhaustion closes with an index, not a better search**: `FactStore` keeps a triple index maintained at `add()` (by tail, by subject, by object for the backward walk), `_walk` looks up first and falls back to cosine + tau_ret only for what the index misses; ambiguity at 0.8% makes the frontier walk nearly free. Object-side retrieval and `tau_ret` 0.50 become moot for template facts (cosine stays for paraphrase, world routing, the flat fallback). The big lever is the parser: 182/246. **Built + VM-verified the same day** (`--lookup` arm of the harvest runner, `exp_m3_cot_pipeline_lookup.*`): 568 verified / 564 correct vs rb1's 517 / 513, +51 and 0 lost, precision 0.9930, every hop from the index; the frontier walk's remaining case is 13 ambiguous-hop questions |
